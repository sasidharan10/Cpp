...
public:
    ...
    bool hasCycle2(ListNode *head)
    {
        ...
        while (cur)
        {
            if (seen.find(cur) != seen.end()) // node visited before → cycle exists
                return true;
            seen.insert(cur); // mark node as visited
            cur = cur->next;
        }
...
- Traverse list, store visited nodes in unordered_set.
- If you revisit a node → cycle detected.
