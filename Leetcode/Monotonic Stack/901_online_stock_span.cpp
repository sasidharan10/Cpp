...
Youtube: https://www.youtube.com/watch?v=IDC1rnr3vus

algorithm:

- We use monotonic stack(maintains increasing order).
- we store previous prices and their span in the stack, in creasing order.
- if st.top()'s price <=  cur price, then it satisfies the "<=" condition
  hence its span will be increased and we pop the elements until the condition
  becomes false.
- Once we exit the loop, we store the current price, and its span in the stack.
...
[7,2,1,2] and the price of the stock today is 2, then the span
of today is 4 because starting from today, the price of the
