...
int main()
{
    ...
    cout << "Number of Laser Beams in a Bank: " << s.numberOfBeams(bank) << endl;
    return 0;
}
...
- Laser beams can be formed in between 2 cells, only if they are on different
  row and no other cells between them.
- To find the count of lasers between any 2 row = countOnes(prev) * countOnes(cur).
- Now, we only update prev = cur, when cur has some 1's in it, or else
  no beam can be formed between them, and that row will be ignored.
- We check if next row has some 1's, if yes, then these 2 rows can make beams.
