...
public:
    ...
    long long totalCost2(vector<int> &costs, int k, int candidates)
    {
        ...
        for (int i = n - 1; i > l && (n - i - 1) < candidates; i--)
        {
            pq.push({costs[i], i});
            r = i;
        }
...
int main()
{
    ...
    cout << "Minimum Cost to Hire K Workers: ";
...
Youtube: https://www.youtube.com/watch?v=ODuICq8exLo
...
smallest index [3,2,7,7,2]. Please note that the indexing may
...
The lowest cost is 1, and we break the tie by the smallest index,
which is 0. The total cost = 0 + 1 = 1. Notice that workers with
